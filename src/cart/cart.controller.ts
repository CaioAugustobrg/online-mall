...
export class CartController {
  ...
  async index(@Req() request) {
    ...
    const viewData = [];
    viewData['title'] = 'Cart - Online Mall';
  ...
  async purchase(@Req() request, @Res() response) {
    if (!request.session.user) {
      ...
      if (total <= user.balance) {
        ...
        request.session.products = null;
        const viewData = [];
        viewData['title'] = 'Purchase - Online Mall';
        viewData['subtitle'] = 'Purchase Status';
        viewData['orderId'] = order.getId();
        return response.render('cart/purchase', { viewData: viewData });
      } else {
        const flashError = request.session.flashErrors;
        request.session.flashErrors = null;
        return response.render('cart/purchase', { flashError });
      }
    }
  }
}
